...
2. **Phân tích Hiệu suất (Performance Analysis) & Dọn dẹp (Cleanup/Push):**
   ...
   - **BÁO CÁO TỔNG QUAN:** HÃY cập nhật một file `training_report.md` (nằm trong thư mục gốc của cấu hình, ví dụ: `workspaces/CFG_XAG_ASIAN_V3_5/training_report.md`). Ghi ngắn gọn tóm tắt nội dung của từng lượt thực hiện (Run ID, tham số thay đổi, kết quả đạt được, và quyết định tiếp theo). Nhớ đẩy file `training_report.md` này lên Hugging Face cùng với các lượt chạy tốt để theo dõi toàn bộ quá trình đào tạo.

3. **Đánh giá & Điều chỉnh Features (Feature Engineering):**
   - **Phiên ASIAN (Châu Á):** XAG chịu ảnh hưởng bởi thị trường Nhật Bản, Trung Quốc. Đặc thù biên độ hẹp (Range-bound), phù hợp với Mean Reversion. 
   - Các chỉ số đang có: XAUUSD, USDJPY, DXY... USDJPY đặc biệt quan trọng trong phiên này vì là Leader thanh khoản của châu Á.
   - *Chiến lược mới (Pivot):* Đánh nhanh rút gọn (TP/SL nhỏ: 15-20 pips). Giữ mạng bộ não nhỏ gọn nhưng vừa đủ (D_MODEL=32) để xử lý vĩ mô. BẮT BUỘC `ZERO_NOISE_TARGET: true` để lọc sideway giả.

4. **Ra quyết định Siêu tham số (Hyperparameter Strategy):**
   - Không gian tìm kiếm (Search Space) tập trung TỐI ĐA VÀO WIN RATE:
     + `WINDOW_SIZE`: Duy trì quanh mức nhỏ (10-20) vì không cần view quá dài cho đánh ngắn.
     + `TP_PIPS`, `SL_PIPS`: Nên giữ ở mức 25-30 pips.
     + `D_MODEL`, `NUM_LAYERS`, `N_HEAD`: Giữ cấu hình tối giản (`D_MODEL: 16` hoặc `32`).
...
5. **Chuyển hướng Chiến lược (Pivot Strategy):**
   ...
   - THÌ BẮT BUỘC KHÔNG ĐƯỢC TẮT TASK. Thay vào đó, hãy **NGHĨ RA MỘT CHIẾN LƯỢC HOÀN TOÀN MỚI** (Ví dụ: thay đổi lớn về tỷ lệ TP/SL, thêm bớt feature vĩ mô) để tiếp tục vòng lặp thử nghiệm. Hãy ghi rõ vào file `tuning_notes.txt` về sự thay đổi chiến lược này để tiện theo dõi.
...
```
3. Commit Git `auto-tuning XAG ASIAN data ready: <RUN_ID>`.
...
Sửa trường `nextRunTime` của task `xag_asian_auto_tuning` trong file `.agent/tasks.json` thành `(timestamp hiện tại + N * 60) * 1000`. Nếu không sửa, Extension sẽ tự lặp lại theo `intervalMinutes` mặc định.
