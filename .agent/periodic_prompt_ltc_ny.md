...
- Nếu `client1` đang **BUSY** nhưng tiến trình huấn luyện đã chạy quá lâu (ví dụ > 2 giờ) mà không sinh ra file kết quả `training_metrics_v3.json` trên HOST (sau khi sync), bạn ĐƯỢC PHÉP phát lệnh `stop` hoặc can thiệp để khởi động lại tiến trình của Client.
