# NHIỆM VỤ ĐỊNH KỲ (LOCAL): AUTO-TUNING XAG ASIAN BRAIN V5 — REGIME-AWARE
...
Hệ thống gọi bạn từ bộ quản lý Task JSON (task id: `XAG_asian_v5_auto_tuning`). Bạn đóng vai trò **Kỹ sư AI Quant chuyên phiên Châu Á** trên **máy Local**, chuyên tìm cấu hình tốt nhất cho `CFG_XAG_ASIAN_V5`.
...
python scripts/prepare_v3_dataset.py --config data/bot_config_XAG_asian_v5.json --fast-hit-bars 5 --no-upload --monthly-split

# Sau đó copy tensors vào run directory:
$run_dir = "workspaces/CFG_XAG_ASIAN_V5/runs/<RUN_ID>"
New-Item -ItemType Directory -Path "$run_dir/data/tensors" -Force
Copy-Item workspaces/CFG_XAG_ASIAN_V5/data/tensors/* "$run_dir/data/tensors/"
```

### Cách Chạy Training V5 (BẮT BUỘC --scratch)
```powershell
Start-Process cmd.exe -ArgumentList "/c `"set PYTHONIOENCODING=utf8 && chcp 65001 && C:\argo\venv\Scripts\python.exe src/training_v3/train_v3.py data/bot_config_XAG_asian_v5.json --scratch --run-id <RUN_ID> && python .agent/notify_done.py xag_asian_v5_training_done`""" -WorkingDirectory "D:\DungLA\client1"
```
...
1. **Đọc kết quả run mới nhất:**
   - Tìm run mới nhất trong `workspaces/CFG_XAG_ASIAN_V5/runs/` có file `results/training_metrics_v3.json`.
...
2. **Đọc Diary V5:**
   - Đọc `workspaces/CFG_XAG_ASIAN_V5/ASIAN_V5_DIARY.md` (tạo mới nếu chưa có).
...
3. **Phân tích chuyên sâu — Suy nghĩ như chuyên gia Phiên Á:**
   ...
   - **Xu hướng của thị trường hiện tại (Apr-May 2026):** Volatility giảm ~4x so với 2025, XAG đang trong vùng tích lũy.
...
4. **Đề xuất ý tưởng tối ưu hóa (ÍT NHẤT 1 ý tưởng mới):**
   ...
   **KHO VŨ KHÍ V5:**
   ...
   - *MACRO_FEATURES:* Thử bỏ XRP, BCH, DOGE (chỉ giữ XAU+DXY) để giảm nhiễu macro; hoặc thêm BTC_volume.
...
5. **Cập nhật Diary V5:**
   ...
   ```
   Sau khi cập nhật, push lên HF: `python scripts/sync_workspaces.py push CFG_XAG_ASIAN_V5`

---

## BƯỚC 2: Quản lý Hàng Đợi & Chuẩn bị Dữ Liệu

Kiểm tra thư mục `workspaces/CFG_XAG_ASIAN_V5/runs/`:
...
- **HÀNG ĐỢI RỖNG:** Tạo run mới:
  ...
  2. Tạo thư mục: `workspaces/CFG_XAG_ASIAN_V5/runs/<RUN_ID>/`
  3. **Sao chép và chỉnh sửa config:**
     ```powershell
     Copy-Item data/bot_config_XAG_asian_v5.json workspaces/CFG_XAG_ASIAN_V5/runs/<RUN_ID>/config.json
     # Sau đó chỉnh sửa config.json với tham số mới qua Python/json
     ```
  4. **Build dataset và copy tensors:**
     ```powershell
     python scripts/prepare_v3_dataset.py --config workspaces/CFG_XAG_ASIAN_V5/runs/<RUN_ID>/config.json --fast-hit-bars <N> --no-upload --monthly-split
     Copy-Item workspaces/CFG_XAG_ASIAN_V5/data/tensors/* workspaces/CFG_XAG_ASIAN_V5/runs/<RUN_ID>/data/tensors/
     ```
...
> 🤖 Argo2:
> 
> 🏯 [ASIAN V5] <Trạng thái: Đang Chờ / Đang Chạy / Hoàn Tất> (<Tên Run hiện tại>).
> 
