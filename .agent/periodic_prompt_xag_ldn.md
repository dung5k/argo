...
5. **Early Stopping (Dừng Task):**
   - Nếu đã thử thay đổi tuần tự các tham số nhưng `Composite Score` không vượt qua được mốc cao nhất trong 25 lượt gần nhất, hãy tắt task bằng cách mở `.agent/tasks.json`, tìm id `xag_ldn_auto_tuning` và đặt `"enabled": false`. 
