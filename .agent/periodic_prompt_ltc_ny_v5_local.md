...
Hệ thống gọi bạn từ bộ quản lý Task JSON (task id: `ltc_ny_v5_auto_tuning`). Bạn đóng vai trò **Kỹ sư AI Quant chuyên phiên New York** trên **máy Local**, chuyên tìm cấu hình tốt nhất cho `CFG_LTC_NY_V5`.
...
- **Config gốc:** `data/bot_config_ltc_ny_v5.json`
- **Workspace:** `workspaces/CFG_LTC_NY_V5/`
- **Raw data:** `workspaces/CFG_LTC_NY_V5/data/raw/`
- **Tensor base:** `workspaces/CFG_LTC_NY_V5/data/tensors/`
- **Diary:** `workspaces/CFG_LTC_NY_V5/NY_V5_DIARY.md`
...
| TP/SL | 0.3%/0.3% | **0.5%/0.4%** | Phù hợp hơn với volatility của phiên New York |

### Cách Build Dataset V5 (BẮT BUỘC dùng flag --monthly-split)
```powershell
# Build dataset với Monthly 2/3-1/3 split
python scripts/prepare_v3_dataset.py --config data/bot_config_ltc_ny_v5.json --fast-hit-bars 5 --no-upload --monthly-split

# Sau đó copy tensors vào run directory:
$run_dir = "workspaces/CFG_LTC_NY_V5/runs/<RUN_ID>"
New-Item -ItemType Directory -Path "$run_dir/data/tensors" -Force
Copy-Item workspaces/CFG_LTC_NY_V5/data/tensors/* "$run_dir/data/tensors/"
```

### Cách Chạy Training V5 (BẮT BUỘC --scratch)
```powershell
Start-Process cmd.exe -ArgumentList "/c `"set PYTHONIOENCODING=utf8 && chcp 65001 && C:\argo\venv\Scripts\python.exe src/training_v3/train_v3.py data/bot_config_ltc_ny_v5.json --scratch --run-id <RUN_ID> && python .agent/notify_done.py ltc_ny_v5_training_done`""" -WorkingDirectory "D:\DungLA\client1"
```
...
1. **Đọc kết quả run mới nhất:**
   - Tìm run mới nhất trong `workspaces/CFG_LTC_NY_V5/runs/` có file `results/training_metrics_v3.json`.
...
2. **Đọc Diary V5:**
   - Đọc `workspaces/CFG_LTC_NY_V5/NY_V5_DIARY.md` (tạo mới nếu chưa có).
...
3. **Phân tích chuyên sâu — Suy nghĩ như chuyên gia Phiên New York:**
   Phiên New York (13:00-21:00 UTC) có đặc điểm:
   - **Thanh khoản và Volatility tăng mạnh:** Sự kiện mở cửa chứng khoán Mỹ, giá chạy có xu hướng rõ ràng hơn.
   - **Breakouts & Momentum:** Các cú phá vỡ (breakout) thường đi xa hơn so với phiên London.
   ...
   - **FAST_HIT_BARS** hiện tại có phù hợp với tốc độ của New York không?

4. **Đề xuất ý tưởng tối ưu hóa (ÍT NHẤT 1 ý tưởng mới):**
   
   **KHO VŨ KHÍ V5:**
   - *FAST_HIT_BARS:* Thử 3, 5, 8 — New York nhanh nên có thể cần bộ lọc thời gian ngắn hơn để bắt sóng gắt.
   - *TP/SL:* Thử 0.005/0.004, 0.006/0.004, 0.007/0.005 — Volatility cao cho phép TP rộng hơn.
   - *MACRO_FEATURES:* Quan trọng! ETHUSDT và BTCUSDT thường dẫn dắt xu hướng phiên New York.
...
5. **Cập nhật Diary V5:**
   ...
   ```
   Sau khi cập nhật, push lên HF: `python scripts/sync_workspaces.py push CFG_LTC_NY_V5`

---

## BƯỚC 2: Quản lý Hàng Đợi & Chuẩn bị Dữ Liệu

Kiểm tra thư mục `workspaces/CFG_LTC_NY_V5/runs/`:
...
- **HÀNG ĐỢI RỖNG:** Tạo run mới:
  1. Sinh RUN_ID: `run_YYYYMMDD_HHMMSS_v5_ny_<tên_ý_tưởng>` (VD: `run_20260507_120000_v5_tp06_fasthi5`)
  2. Tạo thư mục: `workspaces/CFG_LTC_NY_V5/runs/<RUN_ID>/`
  3. **Sao chép và chỉnh sửa config:**
     ```powershell
     Copy-Item data/bot_config_ltc_ny_v5.json workspaces/CFG_LTC_NY_V5/runs/<RUN_ID>/config.json
     # Sau đó chỉnh sửa config.json với tham số mới qua Python/json
     ```
  4. **Build dataset và copy tensors:**
     ```powershell
     python scripts/prepare_v3_dataset.py --config workspaces/CFG_LTC_NY_V5/runs/<RUN_ID>/config.json --fast-hit-bars <N> --no-upload --monthly-split
     Copy-Item workspaces/CFG_LTC_NY_V5/data/tensors/* workspaces/CFG_LTC_NY_V5/runs/<RUN_ID>/data/tensors/
     ```
...
> - Kết thúc: `python .agent/send_to_tele.py "🦅 [New York V5] <tóm tắt ý tưởng>. Run <RUN_ID> đã khởi động. (Best so far: Score X, WR Y%)" --done`
