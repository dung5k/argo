...
5. **Early Stopping (Dừng Task):**
   - Nếu đã thử thay đổi tuần tự nhưng `Composite Score` không vượt qua Baseline trong **25 lượt gần nhất**, hãy tắt task: sửa `.agent/tasks.json`, tìm id `ltc_ny_auto_tuning_local` và đặt `"enabled": false`. Báo cáo Telegram và gọi `--done`.
