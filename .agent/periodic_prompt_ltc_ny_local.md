# NHIỆM VỤ ĐỊNH KỲ (LOCAL): AUTO-TUNING LTC NY BRAIN (CỤC BỘ)
...
4. **Dừng Task**: Nếu đã thử mọi cách nhưng Composite Score không cải thiện qua 25 lượt, hãy tắt task này bằng cách sửa file `.agent/tasks.json`: tìm task có id tương ứng (ví dụ `ltc_ny_auto_tuning_local`) và đặt `"enabled": false`. Báo cáo Telegram và gọi `--done`.
...
- CHỈ KHI HÀNG ĐỢI HOÀN TOÀN RỖNG (0 runs), bạn MỚI CẦN TẠO 1 RUN MỚI:
  ...
  2. Tạo thư mục `workspaces/CFG_LTC_NY_V3_5/runs/<RUN_ID>/` và copy `base_config.json` thành `config.json`. Áp dụng giả thuyết từ Bước 2 vào `config.json` mới. **BẮT BUỘC KHÔNG SỬA `base_config.json` GỐC!**
