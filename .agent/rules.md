...
# Bảo mật mã nguồn (Source Code Security)
**MANDATORY RULE FOR AI AGENT:**
- NGHIÊM CẤM thực hiện `git add`, `git commit` hay `git push` đối với các file cấu hình nhạy cảm mang tính chất local của mỗi máy, đặc biệt là:
  1. `.agent/tasks.json`
  2. `.vscode/settings.json`
- Việc vô tình đưa các file này lên Github (hoặc HuggingFace) sẽ làm sai lệch định danh Agent (`agentIdentity`) và kích hoạt nhầm các nhiệm vụ (Tasks) trên các máy khác khi họ pull code về, dẫn đến HẬU QUẢ CỰC KỲ NGHIÊM TRỌNG (cạnh tranh tài nguyên, ghi đè weights mô hình, spam Telegram).
- Luôn kiểm tra kỹ danh sách `git status` trước khi commit. Nếu lỡ thêm các file này vào staging, phải dùng `git restore --staged <file>` để loại bỏ ngay lập tức!

# Nguyên tắc Trade LTC
- NGHIÊM CẤM sử dụng MT5 (MetaTrader 5) đối với các lệnh trade LTC. LTC phải chạy hoàn toàn trên Binance (hoặc nền tảng khác theo yêu cầu, tuyệt đối không dùng MT5).
