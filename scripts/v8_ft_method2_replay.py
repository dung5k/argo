...
print("=== METHOD 2: REPLAY BUFFER MEMORY ===")
...
# Load old data (Split 20)
print("Loading old data (Split 20) for Replay Buffer...")
old_df = pd.read_parquet("data/v8_splits/split_20_train.parquet")
# Downsample old data so it's roughly equal to the 4-month 2026 data
# Split 20 might be large, we just take the last 4 months of it, or just use it all.
old_h1 = old_df.resample('1h').agg({'open':'first', 'high':'max', 'low':'min', 'close':'last', 'volume':'sum'}).dropna()
old_h4 = old_df.resample('4h').agg({'open':'first', 'high':'max', 'low':'min', 'close':'last', 'volume':'sum'}).dropna()
old_dataset = V8DatasetBuilder(config, old_df, old_h1, old_h4)
...
# Combine datasets (Replay Buffer)
combined_dataset = ConcatDataset([old_dataset, new_dataset])
new_loader = DataLoader(combined_dataset, batch_size=128, shuffle=True)
...
print(f"\nStarting Finetuning with Replay Buffer (Total Samples: {len(combined_dataset)})...")
epochs = 5
best_loss = float('inf')
...
out_name = f"v8_configs/hall_of_fame/{base_mname}_FTM2_PnL{total_pnl:+.0f}_{ts}.pt"
