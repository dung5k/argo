...
print("=== DIRECTION 2: EWC CONTINUAL LEARNING ===")
...
# Step 1: Compute Fisher Information Matrix on old data (split_17_train)
print("Computing Fisher Matrix on old data (Split 17)...")
old_train = pd.read_parquet("data/v8_splits/split_17_train.parquet")
old_h1 = old_train.resample('1h').agg({'open':'first', 'high':'max', 'low':'min', 'close':'last', 'volume':'sum'}).dropna()
old_h4 = old_train.resample('4h').agg({'open':'first', 'high':'max', 'low':'min', 'close':'last', 'volume':'sum'}).dropna()
old_dataset = V8DatasetBuilder(config, old_train, old_h1, old_h4)
old_loader = DataLoader(old_dataset, batch_size=128, shuffle=True)
...
model.eval()
num_samples = 0
# Compute Fisher using log likelihood
for x_m15, x_h1, x_h4, cont_x, y in old_loader:
    ...
    # EWC uses negative log likelihood of true labels for empirical Fisher
...
for name in fisher:
    fisher[name].data /= num_samples

star_params = {name: param.data.clone() for name, param in model.named_parameters()}
ewc_lambda = 5000.0  # Penalty strength

# Combine split_18 and split_19 for new task
print("Loading new task data (Split 18 + 19)...")
new_train = pd.concat([pd.read_parquet("data/v8_splits/split_18_train.parquet"), pd.read_parquet("data/v8_splits/split_19_train.parquet")])
new_h1 = new_train.resample('1h').agg({'open':'first', 'high':'max', 'low':'min', 'close':'last', 'volume':'sum'}).dropna()
new_h4 = new_train.resample('4h').agg({'open':'first', 'high':'max', 'low':'min', 'close':'last', 'volume':'sum'}).dropna()
new_dataset = V8DatasetBuilder(config, new_train, new_h1, new_h4)
new_loader = DataLoader(new_dataset, batch_size=128, shuffle=True)

test_df = pd.read_parquet("data/v8_splits/split_20_test.parquet")
...
for epoch in range(epochs):
    ...
    for x_m15, x_h1, x_h4, cont_x, y in new_loader:
        ...
        loss_ewc = 0
        for name, param in model.named_parameters():
            loss_ewc += (fisher[name] * (param - star_params[name]) ** 2).sum()
        loss_ewc = (ewc_lambda / 2) * loss_ewc
    ...
    train_ce = total_ce/len(new_loader)
    train_ewc = total_ewc/len(new_loader)
    val_avg = val_loss/len(test_loader)
    total_trades = wins + losses + scratches
    real_wr = (wins / total_trades * 100) if total_trades > 0 else 0
    print(f"Ep {epoch+1} | CE: {train_ce:.3f} | EWC Penalty: {train_ewc:.3f} | Val Loss: {val_avg:.3f}")
    print(f"  -> PnL: {total_pnl:+.1f} | Trades: {total_trades} | WR: {real_wr:.1f}% | W:{wins} L:{losses} S:{scratches}")
...
# Extract base model name without S17_PnL
base_mname = model_name.split('_S17')[0]
import datetime
ts = datetime.datetime.now().strftime("%y%m%d_%H%M")
out_name = f"v8_configs/hall_of_fame/{base_mname}_EWC_PnL{total_pnl:+.0f}_{ts}.pt"
torch.save(model.state_dict(), out_name)
print(f"Saved: {out_name}")

print("=== EWC CONTINUAL LEARNING COMPLETED ===")
