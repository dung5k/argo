...
def main():
    ...
    # Extract num_layers from checkpoint keys
    ...
    print(f"Detected Num Layers: {num_layers}")
    ...
    print(f"Total Test Data Rows: {len(full_df)} (from {full_df.index.min()} to {full_df.index.max()})")
    ...
    print("Simulating PnL (TP=3.0, SL=1.5, Hold=12, Thresh=0.30)...")
    threshold_sim = 0.30
    spread = 0.3
    tp_mult = 3.0
    sl_mult = 1.5
    ...
    for idx in range(n_samples):
        ...
        if result == 'scratch':
            # Kiểm tra đóng lệnh cưỡng bức tại cấp nến M15 (nếu chưa break ở trên)
        ...
        if result == 'win':
            ...
            losses += 1; consecutive_losses += 1; max_consec_losses = max(max_consec_losses, consecutive_losses)
        else:
            scratches += 1
            if pnl < 0: consecutive_losses += 1; max_consec_losses = max(max_consec_losses, consecutive_losses)
    ...
    if total_trades > 0:
        ...
        print(f"Wins         : {wins} ({real_wr:.1f}%)")
        print(f"Losses       : {losses}")
        print(f"Scratches    : {scratches}")
        print(f"Total PnL    : {total_pnl:+.1f} pip")
        print(f"Profit Factor: {pf:.2f}")
        print(f"Max Drawdown : {max_drawdown:.1f} pip")
        print(f"Max Streak   : {max_consec_losses} losses")
    else:
