        ...
        for folder in folders:
            if glob.glob(os.path.join(folder, "*.pth")):
                ...
                if os.path.exists(inherited_cfg):
                    print(f"🔄 [INIT] PHÁT HIỆN KẾ THỪA: Ghi đè cấu hình hiện tại bằng cấu hình từ HF ({inherited_cfg})")
                    with open(inherited_cfg, "r", encoding="utf-8") as f:
                        cfg.update(json.load(f))
                        TARGET_PREFIX = cfg.get("TARGET_PREFIX", TARGET_PREFIX)
                        CONFIG_ID     = cfg.get("CONFIG_ID", CONFIG_ID)
                    config_path = inherited_cfg
                break
    except Exception as e:
        print(f"⚠️ [INIT] Lỗi kiểm tra cấu hình kế thừa: {e}")
    # ==================================
