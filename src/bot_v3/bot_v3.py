...
def _load_brain_metrics(run_id: str, config_id: str, is_active: bool = False) -> str:
...
def _get_all_brains_report() -> str:
    ...
    # Lấy thông tin từ schedule file nếu có
    ...
    for s_name, r_id, c_id in schedule:
        ...
        reports.append(_load_brain_metrics(r_id, c_id, is_active))
        reports.append("\n" + "="*20 + "\n")
...
def bot_background_loop():
    ...
    while True:
        ...
        notify_every_candle = CONFIG.get("LIVE_BOT", {}).get("TELEGRAM_NOTIFY_EVERY_CANDLE", False)
        ...
        if current_brain != bot_background_loop.last_reported_brain:
            ...
            # Gửi tin định kỳ nếu có sự kiện hoặc giãn cách (mặc định cho v3 nếu có action BUY/SELL hoặc cấu hình gửi mọi nến)
            if action in ["BUY", "SELL"] or notify_every_candle:
                tg_notify(msg_pred)
