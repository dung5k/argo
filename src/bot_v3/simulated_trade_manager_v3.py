...
class SimulatedTradeManagerV3:
    ...
    def __init__(self, target_symbol: str, config: dict,
        ...
        self.get_price_fn = get_price_fn  # Hàm trả về giá hiện tại: () -> float
    ...
    def _calc_pnl(self, pos: dict, current_price: float) -> float:
        """P&L chưa thực hiện (Unrealized) tính bằng USDT."""
    ...
    def _open_position(self, side: str, current_price: float,
        ...
        self.log_callback(f"[SimTM] 🔥 MỞ LỆNH {side} #{ticket} | Giá: {current_price:.4f} | SL: {sl_price:.4f} | TP: {tp_price:.4f} | Qty: {qty}")
        self.tg_notify(msg)
        return ticket
    ...
    def _close_position(self, ticket: int, reason: str, current_price: float):
        ...
        msg = (
            ...
            f"📊 Tích lũy: {self.total_pnl_usdt:+.2f} USDT | "
            f"WR: {wr_pct:.1f}% ({self.win_trades}/{self.total_trades})"
        )
    ...
    def execute_trade(self, action: str, probs_dict: dict,
        ...
        for ticket in list(self.active_trade_loggers.keys()):
            ...
            if pos["side"] == "BUY" and action == "SELL":
                ...
                pnl = self._calc_pnl(pos, current_price)
                pnl_icon = "🟢" if pnl >= 0 else "🔴"
                self.gui_action = f"[SIM] GIỮ {pos['side']} #{ticket} {pnl_icon} {pnl:+.2f}$"
    ...
    # ------------------------------------------------------------------
    # Báo cáo vị thế hiện tại (hiển thị trên GUI & Telegram)
    # ------------------------------------------------------------------
    def get_active_positions_report(self) -> str:
        ...
        wr_pct = (self.win_trades / self.total_trades * 100) if self.total_trades > 0 else 0.0
        footer = (
            f"  {pnl_total_icon} P&L hiện tại: {total_pnl:+.2f} USDT\n"
            f"  📈 Tích lũy: {self.total_pnl_usdt:+.2f} USDT | "
            f"WR: {wr_pct:.1f}% ({self.win_trades}/{self.total_trades})"
        )

        return "🎭 [SIM] Vị thế giả lập:\n" + "\n".join(lines) + "\n" + footer
