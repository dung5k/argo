...
# THÊM PIPELINE V2 CHO KIẾN TRÚC MỚI
try:
    from src.training_v2.feature_pipeline_v2 import FeaturePipelineV2
except Exception as ep:
    print(f"Cảnh báo: Không nạp được FeaturePipelineV2: {ep}")
...
def bot_background_loop():
    ...
    # Ở phiên bản V2, chúng ta ko cần đếm cứng num_features từ file scaler cũ nữa
    # Tuy nhiên ta cứ lấy fallback từ parquet nếu có thể, hoặc để lúc bot_logic load trọng số tự động đếm.
    ...
    try:
        ...
        num_features = 86  # Fallback tuyệt đối cho XAUUSD
        log_message(f"[BOT] Fallback: Không thể xác định chính xác từ file, dùng Base={num_features}.")
    ...
    while True:
        ...
        # Dịch session_id sang prefix (asia, london, ny)
        s_map = {"asian": "asia", "european": "london", "us": "ny"}
        session_str = s_map.get(session_id, "unified")
        
        # Override tên file nếu cấu hình là V1 cũ
        if "v2_weights_" in WEIGHT_FILE:
            WEIGHT_FILE = WEIGHT_FILE.replace("v2_weights_", f"{session_str}_weights_")
        elif "unified_weights" in WEIGHT_FILE:
            WEIGHT_FILE = WEIGHT_FILE.replace("unified_weights.pth", f"{session_str}_weights_BEST_VLOSS.pth")
        ...
        if session_id != current_loaded_session:
            ...
            try:
                ...
                if hf_run_cfg:
                    ...
                    log_message(f"[HF CLOUD] XÁC ĐỊNH NÃO BỘ CHỈ ĐỊNH: {latest_run} (File: {WEIGHT_FILE})")
                else:
                    ...
                    log_message(f"[HF CLOUD] TỰ ĐỘNG DÒ TÌM NÃO BỘ MỚI NHẤT: {latest_run}")
                ...
                # ĐỒNG BỘ SCALER V2 KHỚP VỚI TRỌNG SỐ (CỰC QUAN TRỌNG)
                try:
                    ...
                    scaler_local = os.path.join(script_dir, "data", "scaler_v2.pkl")
                    shutil.copy(scaler_cloud_path, scaler_local)
                    mt5_manager.reload_features() # Xây lại lưới ngay lập tức để hít đúng Data
                    log_message(f" ├─ ✅ [SCALE SHIELD] Đã đồng bộ Scaler V2 Local về đúng định dạng của não {latest_run}!")
                except Exception as sce:
                    log_message(f" ├─ ⚠️ Không thể tải đồng bộ scaler_v2.pkl từ đám mây: {sce}")
                ...
                try:
                    ...
                    with open(metrix_path, "r", encoding='utf-8') as fm:
                        metrix = json.load(fm)
                    feats = metrix.get("data_features", [])
        ...
        if True:
            ...
            try:
                ...
                # 1. Tính toán Đặc trưng Động (Log Return, RSI, Quán Tính Đa Khung)
                # Đặt is_live=False để XẢ BỎ BƯỚC NẠP scaler V1 (Z-Score)
                df, _ = fe.create_stationary_features(merged_df, is_live=False)
                
                # 2. Xử lý V2 (QuantileTransformer): Ép đuôi béo về Chuẩn Hoá
                safe_script_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
                v2_data_dir = os.path.join(safe_script_dir, "data")
                pipeline_v2 = FeaturePipelineV2(target_prefix=TARGET_PREFIX, data_dir=v2_data_dir)
                pipeline_v2.load_scaler()
                df = pipeline_v2.transform(df)
                
                dt_feat = time.time() - t0_feat
                log_message(f" ├─ Trạm Lõi lượng tử (RAM Mapped V2): ✅ HOÀN TẤT ({dt_feat:.2f}s)")
                log_message(f" 📊 KIỂM KÊ KHO: RAM Nạp thành công {len(df):,} nến.")
                
                active_inference_feats = pipeline_v2._feature_names
