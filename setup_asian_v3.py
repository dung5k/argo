...
run_id = f'run_{run_timestamp}_v6_ASIAN_1m_W20_STF_FixParams'
...
# Cleanup old wrong keys if any exist in base (though we loaded from fresh base so it's clean)
...
if 'MTF_INPUTS' in config and len(config['MTF_INPUTS']) > 0:
    ...
    config['MTF_INPUTS'] = [config['MTF_INPUTS'][0]] # Remove other timeframes
