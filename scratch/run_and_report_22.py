...
def main():
    ...
    run_id = "run_20260427_192911_v4_ldn_22"
    session = "ldn"
    ...
    if os.path.exists(metrics_path):
        ...
        report = f"KẾT QUẢ CHIẾN LƯỢC 3 (Macro Momentum Burst):\n"
        ...
        if score < 0.65 or win_rate < 80.0:
            ...
            report += f"Với Win Rate {win_rate:.2f}% và Composite {score:.4f}, mang mô hình này ra thực chiến CHẮC CHẮN SẼ CHÁY TÀI KHOẢN. "
            report += f"Việc tăng WINDOW_SIZE=60 và D_MODEL=64 dường như không giúp AI nhận diện sự phân kỳ tốt hơn, hoặc dữ liệu M1 bị nhiễu quá nhiều. Đề xuất luân phiên tiếp sang Chiến lược 4 (Micro-Scalping)."
        else:
