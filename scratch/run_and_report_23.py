...
def main():
    ...
    run_id = "run_20260427_193351_v4_ldn_23"
    session = "ldn"
    ...
    if os.path.exists(metrics_path):
        ...
        report = f"KẾT QUẢ CHIẾN LƯỢC 4 (Smart Money Micro-Scalping):\n"
        ...
        if score < 0.65 or win_rate < 80.0:
            ...
            report += f"Với Win Rate {win_rate:.2f}% và Composite {score:.4f}, mang mô hình này ra thực chiến CHẮC CHẮN SẼ CHÁY TÀI KHOẢN do chi phí giao dịch (Spread) nuốt trọn TP 20 pips. "
            report += f"Mạng AI nhỏ (D_MODEL=16) không đủ sức chứa rủi ro. Chúng ta đã luân phiên qua cả 4 Chiến lược nhưng London Session vẫn nhiễu quá mạnh. Cần xem xét lại việc gộp 2 phiên hoặc đổi mô hình phân tích cơ bản."
        else:
