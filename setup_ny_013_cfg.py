...
run_id = f"run_{datetime.now().strftime('%Y%m%d_%H%M%S')}_v6_NY_Cross5m_Win120_013"
run_dir = os.path.join("workspaces", "CFG_LTC_NY_V6", "runs", run_id)
os.makedirs(os.path.join(run_dir, "data", "tensors"), exist_ok=True)
os.makedirs(os.path.join(run_dir, "results"), exist_ok=True)

# Apply Search Space for Seed 013 (Same as 012)
...
# MTF_INPUTS: LTCUSDT 5min (W120) + BTCUSDT 5min (W120)
