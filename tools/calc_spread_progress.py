...
def is_valid_group(path):
    ...
    p = path.lower()
    # Loại bỏ cổ phiếu cá biệt để chạy siêu nhanh, chỉ lấy các thị trường thanh khoản lớn
...
for t_name, t_path in terminals.items():
    ...
    for s in filtered_symbols:
        count += 1
        # In tiến độ liên tục trên cùng 1 dòng
        print(f"\r  Đang quét: {count}/{total} mã | Thời gian: {round(time.time() - t_start, 1)}s | Mã: {s.name:<15}", end="", flush=True)
...
if not df_res.empty:
    ...
    # Tạo Artifact (Báo cáo CSV)
    csv_path = os.path.abspath("spread_analysis_24h.csv")
    df_res.to_csv(csv_path, index=False, encoding='utf-8-sig')
    
    print("\n" + "="*70)
    print("🏆 BẢNG VÀNG: TOP 25 MÃ GIAO DỊCH TỐT NHẤT THẾ GIỚI CHO SCALPING M5")
    print("="*70)
    ...
    print(f"\n📁 Đã xuất toàn bộ dữ liệu chi tiết của {len(df_res)} mã ra file Excel: {csv_path}")
    print("Mở file CSV để tự sắp xếp theo nhóm yêu thích!")
else:
